...
def test_classifier_source_declares_information_bottleneck_heads():
    ...
    assert "self.audio_clf" in source
    assert "self.video_clf" in source
    assert "def _sample_features" in source
...
def test_classifier_returns_information_bottleneck_outputs(monkeypatch):
    ...
    assert audio_latent.shape == (2, 512)
    assert video_latent.shape == (2, 512)
...
def test_eps_scale_zero_uses_mean_logits_during_training(monkeypatch):
    ...
    _, audio_logits, video_logits, audio_latent, video_latent, _ = model(audio, video)
    audio_features = model.audio_encoder(audio)
    video_features = model.video_encoder(video)

    assert torch.allclose(audio_latent, model.audio_mu(audio_features))
    assert torch.allclose(video_latent, model.video_mu(video_features))
    assert torch.allclose(audio_logits, model.audio_clf(audio_latent))
    assert torch.allclose(video_logits, model.video_clf(video_latent))
