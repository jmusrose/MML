...
# -*- coding: utf-8 -*-
#
# Common utilities for the DML_v1/CMU project.
#
# Style mirrors DML_v1/RGB_v1/utils/utils.py so the two datasets share the
# same low-level toolchain. Provides:
#   - Averager: running-mean accumulator used by the training loop.
#   - set_seed: seeds all PRNGs (random / numpy / torch / cuda) and forces
#     deterministic cuDNN behaviour for reproducibility.
#   - save_checkpoint / load_checkpoint: torch.save based checkpoint helpers.
#   - append_experiment_record: append a record to the per-dataset experiment
#     summary JSON file (best-effort, no file locking).
...
def load_checkpoint(model, path):
    ...
    ``{'epoch', 'model_state_dict', 'optimizer_state_dict', 'metrics', 'clean_acc'}``
    as well as the legacy ``{'state_dict': ...}`` layout. If neither key is
