...
The current DML_v1/CMU baseline uses ``torch.optim.lr_scheduler.ReduceLROnPlateau``
configured directly inside the main entry scripts (``DML_mosi.py`` /
``DML_mosei.py``), so nothing in this file is wired into the pipeline yet.

A ``WarmupMultiStepLR`` template is provided below for future experiments
(e.g. when switching to a step-based schedule with linear warmup). Mirrored
from ``DML_v1/RGB_v1/tool/lr_scheduler.py``.
"""
