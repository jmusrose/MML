...
- ``args.name = f"dml_mosei_seed{seed}_lr{lr}"``
- 引用 ``models.dml_classifier_mosei.Classifier``
"""
...
from models.dml_classifier_mosei import Classifier
...
def get_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--savedir",
        type=str,
        default=os.path.join(_THIS_DIR, "savepath", "mosei"),
    )
...
def main():
    """CMU-MOSEI 完整训练-保存-鲁棒评估流水线。

    与 ``DML_mosi.main()`` 的镜像实现，差异仅在主入口默认 CLI 与
    ``args.name`` / ``args.dataset`` 标识、引用的 ``Classifier`` 来自
    ``models.dml_classifier_mosei``。
    ...
    1. 设置 ``CUDA_VISIBLE_DEVICES``、解析 CLI、用 seed/lr 拼接
       ``args.name = f"dml_mosei_seed{seed}_lr{lr}"``，并把 ``args.savedir``
    ...
    parser = argparse.ArgumentParser(
        description="Train DML CMU-MOSEI Multimodal Sentiment Regression Model"
    )
    get_args(parser)
    ...
    args.name = f"dml_mosei_seed{args.seed}_lr{args.lr}_blr{args.bert_lr}"
    ...
    # 8) 追加到该数据集的总实验记录（dirname(savedir) 即 ./savepath/mosei/）
