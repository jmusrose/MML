...
def test_conformal_uncertainty_weights_use_prediction_set_sizes():
    ...
    logits = {
        "fusion": np.array([[4.0, 0.0], [0.0, 4.0]]),
        "rgb": np.array([[4.0, 0.0], [0.0, 4.0]]),
        "depth": np.array([[4.0, 0.0], [0.0, 4.0]]),
    }
    labels = np.array([0, 1])
    tau = 2.0
    ...
    metrics = evaluate_conformal(
        logits,
        labels,
        {"fusion": float("inf"), "rgb": 0.0, "depth": 0.2},
        tau=tau,
    )

    expected_reliability = {
        "fusion": math.exp(-tau * 1.0),
        "rgb": math.exp(-tau * 0.0),
        "depth": math.exp(-tau * 0.5),
    }
    denom = sum(expected_reliability.values())

    assert metrics["fusion"]["avg_uncertainty"] == 1.0
    assert metrics["rgb"]["avg_uncertainty"] == 0.0
    assert metrics["depth"]["avg_uncertainty"] == 0.5
...
def test_conformal_uncertainty_from_logits_reports_average_set_size_ratio():
    ...
    uncertainties = conformal_uncertainty_from_logits(
        logits,
        {"rgb": float("inf"), "depth": 0.2},
    )

    assert uncertainties["rgb"] == 1.0
    assert uncertainties["depth"] == 0.5
