...
class Classifier(nn.Module):
    def __init__(self, args):
        ...
        self.rgb_mu = nn.Linear(d_in, d_in)
        self.rgb_logvar = nn.Linear(d_in, d_in)
        self.rgb_clf = nn.Linear(d_in, args.n_classes)
        self.depth_mu = nn.Linear(d_in, d_in)
        self.depth_logvar = nn.Linear(d_in, d_in)
    ...
    def forward(self, rgb, depth):
        ...
        rgb_latent = self._sample_features(rgb_mu, rgb_logvar)
        depth_latent = self._sample_features(depth_mu, depth_logvar)
        rgb_out = self.rgb_clf(rgb_latent)
        depth_out = self.depth_clf(depth_latent)
        ...
        return both_output, rgb_out, depth_out, rgb_latent, depth_latent, ib_loss
