...
def build_sun_dataloaders(args, train_transform, val_transform):
    ...
    calib_source = AlignedConcDataset(
        args,
        data_dir=os.path.join(args.data_path, 'train'),
    ...
    val_loader = DataLoader(
        Subset(calib_source, calib_indices),
        batch_size=args.batch_sz,
        shuffle=False,
        num_workers=args.n_workers,
    )
...
def main():
    ...
    # 4) 数据加载
    train_loader, val_loader, test_loader, calib_loader = build_sun_dataloaders(
        args,
    ...
    for epoch in range(args.max_epochs):
        ...
        val_acc = val_rgbd(epoch, test_loader, model, logger, args)
