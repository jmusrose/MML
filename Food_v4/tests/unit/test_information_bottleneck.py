#!/usr/bin/env python3
"""Unit tests for Food_v2 information bottleneck heads."""
...
def test_branch_heads_return_logits_features_and_ib_loss(monkeypatch, mock_args):
    ...
    txt_logits, txt_features, txt_ib = txt_model(txt, mask, segment)
    img_logits, img_features, img_ib = img_model(img)

    assert txt_logits.shape == (batch_size, mock_args.n_classes)
    assert img_logits.shape == (batch_size, mock_args.n_classes)
    assert txt_features.shape == (batch_size, mock_args.hidden_sz)
    assert img_features.shape == (
        batch_size,
        mock_args.img_hidden_sz * mock_args.num_image_embeds,
    )
    assert torch.allclose(txt_logits, txt_model.mu(txt_features))
    assert torch.allclose(img_logits, img_model.mu(img_features))
...
def test_eval_mode_uses_mean_logits_without_sampling(monkeypatch, mock_args):
    ...
    first_logits, first_features, _ = model(txt, mask, segment)
    second_logits, _, _ = model(txt, mask, segment)

    assert torch.allclose(first_logits, second_logits)
    assert torch.allclose(first_logits, model.mu(first_features))


def test_classifier_returns_information_bottleneck_outputs(monkeypatch, mock_args):
    ...
    class FakeTextClf(nn.Module):
        ...
        def forward(self, txt, mask, segment):
            ...
            features = torch.ones(txt.shape[0], self.args.hidden_sz)
            return logits, features, torch.tensor(1.25)
    ...
    class FakeImageClf(nn.Module):
        ...
        def forward(self, img):
            ...
            return logits, features, torch.tensor(2.75)
