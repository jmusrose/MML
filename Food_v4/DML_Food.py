#!/usr/bin/env python3
"""
DML_v1/Food_v1/DML_Food.py

Main training script for DML multimodal classification on UPMC Food-101.
...
def get_args(parser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Temperature for conformal set-size uncertainty weighting",
    )
    parser.add_argument(
        "--calib_size",
        type=int,
        default=0,
        help="Calibration sample count; <=0 uses 20% of train split",
    )
    parser.add_argument(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DML Multimodal Classification on Food-101"
    )
    get_args(parser)
    ...
    conformal_results = {
        ...
        "calibration": {
            "source": getattr(args, "calibration_source", "train_split"),
