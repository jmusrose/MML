...
其余 ``train_rgbd`` / ``val_rgbd`` / ``main`` 流水线逻辑（决策级融合 + 三路 CE
损失之和、最佳检查点保存、5 个鲁棒性场景评估、final_results.json 落盘）与
NYU 版本完全一致。

风格参考：``CPSC_RGB/CPSC_sun.py``、``DML_v1/RGB/DML_nyu.py``。
"""
...
from models.dml_classifier_sun import Classifier
...
def get_args(parser: argparse.ArgumentParser) -> None:
    """注册 SUN RGB-D 主入口的全部命令行参数。

    所有参数及默认值与 design.md 的参数表保持一致；``--n_classes`` 默认为
    19（与 NYU 一致），``--img_embed_pool_type`` 受限为 ``{"max", "avg"}``。
...
def compute_mAP(outputs, labels):
    ...
    备注
    ----
    本助手与 ``CPSC_RGB/CPSC_sun.py`` 中的实现等价，主要用于将来支持多标签
...
def train_rgbd(epoch, train_loader, model, optimizer, logger, args):
    ...
    model : nn.Module
        ``models.dml_classifier_sun.Classifier`` 实例（已 ``.cuda()``）。
    ...
    for step, batch in enumerate(tqdm(train_loader, desc=f"Training epoch {epoch}")):
        ...
        both_out, rgb_out, depth_out, _, _ = model(rgb, depth)

        loss_both = criterion(both_out, tgt)
        loss_rgb = criterion(rgb_out, tgt)
        loss_depth = criterion(depth_out, tgt)
        loss = loss_both + loss_rgb + loss_depth

        if torch.isnan(loss).any() or loss <= 0:
            print(f"NaN detected at step {step}")
            print(
                f"loss_both: {loss_both.item()}, "
                f"loss_rgb: {loss_rgb.item()}, "
                f"loss_depth: {loss_depth.item()}"
            )
        ...
        tl.add(loss.item())
        tl_both.add(loss_both.item())
        tl_rgb.add(loss_rgb.item())
        tl_depth.add(loss_depth.item())
        with torch.no_grad():
...
def val_rgbd(epoch, val_loader, model, logger, args):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            both_out, _, _, _, _ = model(rgb, depth)
...
def main():
    ...
    5. 实例化 ``Classifier(args).cuda()``（来自 ``models.dml_classifier_sun``）、
    ...
    parser = argparse.ArgumentParser(description="Train DML RGB-D Scene Recognition Model on SUN RGB-D")
    get_args(parser)
    args = parser.parse_args()

    args.name = f"dml_sun_seed{args.seed}_lr{args.lr}"
    ...
    # 4) 数据加载
    full_train_dataset = AlignedConcDataset(
        args,
    ...
    for name, loader in test_scenarios.items():
        ...
        acc = val_rgbd(-1, loader, clean_model, logger, args)
    ...
    logger.info(
        ...
        f"clean_acc={final_results['Clean Test']:.4f}"
    )

    # 9) 追加到该数据集的总实验记录（dirname(savedir) 即 ./savepath/sun_rgbd/）
