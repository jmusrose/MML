...
class AlignedConcDataset:
    """RGB-Depth concatenated image dataset.

    Each sample on disk is a single image where the left half is the RGB view
    and the right half is the Depth view. ``__getitem__`` splits the image
    along the horizontal midline and returns the two views together with
