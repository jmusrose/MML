...
class AlignedConcDataset:
    """RGB-Depth concatenated image dataset with independent transforms.

    Mirrors :class:`data.aligned_conc_dataset.AlignedConcDataset` but accepts
    two independent transforms: ``rgb_transform`` is applied only to the RGB
    view (``A``) and ``depth_transform`` is applied only to the Depth view
    (``B``). This is used for robustness evaluation where noise can be
    injected into one or both modalities independently.
    """
    ...
    def __getitem__(self, index):
        ...
        if w2 > self.cfg.FINE_SIZE:
            A = AB_conc.crop((0, 0, w2, h)).resize(
                (self.cfg.LOAD_SIZE, self.cfg.LOAD_SIZE), Image.BICUBIC
            )
            B = AB_conc.crop((w2, 0, w, h)).resize(
                (self.cfg.LOAD_SIZE, self.cfg.LOAD_SIZE), Image.BICUBIC
            )
        else:
